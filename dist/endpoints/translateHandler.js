...
export const translateHandler = async (req) => {
    try {
        ...
        if (!adapter) {
            return Response.json({ error: 'Translation adapter not configured', success: false }, { status: 500 });
        }
        ...
        if (!document) {
            return Response.json({ error: 'Document not found', success: false }, { status: 404 });
        }
        const collectionConfig = payload.collections[collection]?.config;
        if (!collectionConfig) {
            return Response.json({ error: 'Collection not found', success: false }, { status: 404 });
        }
